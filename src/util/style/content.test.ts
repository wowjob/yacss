...
describe('normalizeContent', () => {
  ...
  test('should return <counter> values as-is', () => {
    ...
    expect(
      normalizeContent('counter(chapter_counter, upper-roman)'),
    ).toBe('counter(chapter_counter, upper-roman)');
  ...
  test('should return <content-list> values as-is', () => {
    expect(normalizeContent('"prefix" url(http://www.example.com/test.png)')).toBe(
      '"prefix" url(http://www.example.com/test.png)',
    );
    expect(
      normalizeContent('"prefix" url("/img/test.png") "suffix" / "Alt text"'),
    ).toBe('"prefix" url("/img/test.png") "suffix" / "Alt text"');
    expect(
      normalizeContent('open-quote counter(chapter_counter)'),
    ).toBe('open-quote counter(chapter_counter)');
  });
